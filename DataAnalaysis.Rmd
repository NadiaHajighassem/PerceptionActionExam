...
Here i am just loading the packages and setting up our working
directory. I am also specifying, which group we are looking at.
...
# data_dir <- path_home() %>% 
#   path("Documents", "GitHub", "PerceptionActionExam", "data", "tsvs")  #Nadia


data_dir <- path_home() %>% 
  path("Desktop","UNI", "3.semester", "Perception & Action", "PerceptionActionExam", "data", "tsvs") #Katharina
...
And so, some more boring stuff...This is just getting the labels so we
can make sure that the tracked markers match up with what we expected,
really, this shouldn't go wrong, but if you don't check this kind of
thing you'll end up scratching your head later wondering why everything
broke.
...
Finally loading in data hehehe (insert elmo meme with fire in the
background)

Additionally, this code is transforming data from a wide format, where
each marker's x, y, and z coordinates are in separate columns, to a long
format where all the coordinates are in a single column. It then adds
more information about each observation (subject, axis, and marker)
before pivoting it back to a wide format.
...
Here's where you cut the start and ends off of the trajectores, this is
useful for example when you know that there's some setup time or a delay
from recording start to end, or if you just want to look at a specific
section of the data.

If you set up the conditions correctly in the setup section, then you
can just run this cell and it will crop the data for you.
...
I've included some ways here of inspecting the data to see if there are
any obvious problems.

## Check for large jumps in trajectory data

This is a simple check to see if there are any large jumps in the data,
this can indicate that there are some label errors (e.g. label
`hand_left` became label `hand_right`), or that the data is just bad.
...
Specifically, this checks how much of a given trajectory is `NA` values
and how long the longest sequence of `NA` values is. This can be useful
for deciding whether to discard a marker or not.

*Remember, you'll want to make sure that both subject A and B have at
least one good quality matching marker.*
...
TL:DR; There is a problem with group 7 and 10 There is a problem with
loading all the data files in. I will continue with data analysis, so we
can ask Peter --------------------------------------------------------

# Pick your poison

You can choose one or more markers and put them in the
`markers_of_interest` variable below. This will then be used for the
...
Now that we're left with mainly the data we want to work with, we can do
some gap filling to eliminate any `NA` values, just beware that it can't
fill NAs at the start or end of the data.
...
<https://www.rdocumentation.org/packages/fastICA/versions/1.2-4/topics/fastICA>
<https://cran.r-project.org/web/packages/iTensor/vignettes/iTensor-1.html>

In short ICA attempts to 'un-mix' the data by estimating an un-mixing
matrix W where XW = S.

Under this generative model the measured 'signals' in X will tend to be
\`more Gaussian' than the source components (in S) due to the Central
...
neg-entropy () which are more robust than kurtosis-based measures and
fast to compute.

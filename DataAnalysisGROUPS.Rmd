...
joint_lead_combined <- bind_rows(joint_lead_list)
...
lead_follow_combined <- bind_rows(lead_follow_list)

#lead_follow_combined <- bind_rows(lead_follow_list, .id = lead_follow_list$group)
#lead_follow_combined <- bind_rows(lead_follow_list, .id = lead_follow_list$group)
...
for(n in 1:9){
  ...
  selected_traj_data1 <- selected_traj_data %>% 
    group_by(condition, subject, marker) %>% 
    mutate_at(
      vars(x, y, z),
      ~ gap_fill_linear(.)
...
```{r splitting dataframes and creating datamatrices, DONT RUN THIS, RUN THE ONE BENEATH IT INSTEAD}
unique_groups <- unique(selected_traj_data$group)
unique_conditions <- unique(selected_traj_data$condition)
unique_subjects <- unique(selected_traj_data$subject)

# Iterate over unique combinations of group, condition, and subject
for (group in unique_groups) {
  for (condition in unique_conditions) {
    for (subject in unique_subjects) {
      # Filter data based on the current group, condition, and subject
      subset_data <- selected_traj_data[selected_traj_data$group == group & 
                                        selected_traj_data$subject == subject & 
                                        selected_traj_data$condition == condition, ]
      
      # Create a unique identifier for the subset matrix
      subset_identifier <- paste0("matrix_", group, "_", condition, "_", subject)
      
      # Print information about the subset data
      cat("Subset data:", subset_identifier, "Number of observations:", nrow(subset_data), "\n")
      # Convert the subset data frame to a matrix
      #subset_matrix <- data.matrix(subset_data)
      
      # Assign the subset matrix to a new variable in the global environment
      assign(subset_identifier, subset_matrix, envir = .GlobalEnv)
    }
  }
}
```

```{r THIS REMOVES ALL NAS}
# Iterate over unique combinations of group, condition, and subject
unique_groups <- unique(selected_traj_data$group)
unique_conditions <- unique(selected_traj_data$condition)
unique_subjects <- unique(selected_traj_data$subject)
...
Side note: The following groups have zero dataframes, which makes a lot of sense, since we don't have their condition and quite frankly i dont even know why the for loop made them - my theory is that because we do gap filling before splitting the dataframes, the NA's are now replaced with 1's or something else which is why they get printed

matrix_group4_jointlead_A, matrix_group4_jointlead_B,

matrix_group8_jointlead_A, matrix_group8_jointlead_B

So instead of changing my for loop, i will be lazy and just remove them from the enviroment

Also note, we only need this for loop if we are doing ICA!!! because the input has to be a matrix

```{r remove groups}
rm(matrix_group4_jointlead_A, matrix_group4_jointlead_B, matrix_group8_jointlead_A, matrix_group8_jointlead_B)
```

## PCA ##
Here is what we need to do for PCA
1. Make sure that the dataframe only contains numeric values
2. Check for NA's
3. Normalize the data
4. Plot the covariance/correlation matrix
5. Do the actual PCA and then check cumulative proportion
6. Create additional plots to gain further insight

```{r downloading required packages}
pacman::p_load(
  corrr,
  ggcorrplot,
  FactoMineR,
...
```{r}
## inspect data
head(group2_jointlead)

## remove index,time, group and condition, as they are not relevant for our analysis
group2_jointlead.PCA <- group2_jointlead[,-c(1,2,3,4)]

## make into factor
group2_jointlead.PCA$marker <- as.factor(group2_jointlead.PCA$marker)
group2_jointlead.PCA$subject <- as.factor(group2_jointlead.PCA$subject)

#convert marker and subject variables to numeric
group2_jointlead.PCA[, c('marker', 'subject')] <- sapply(group2_jointlead.PCA[, c('marker', 'subject')], unclass)

# check the new dataframe
head(group2_jointlead.PCA)
## i am actually not sure whether we need marker or subject actually?
```

2. Check for NA's
```{r}
sum(is.na(group2_jointlead.PCA))

## temporary solution
group2_jointlead.PCA <- na.omit(group2_jointlead.PCA)
```
3. Normalize the data
```{r}
data_normalized <- scale(group2_jointlead.PCA) 
...
## Most of the data can be explained by the two first components, which is really nice!!!
```
```{r}
## Here we can see what the components consists of and how much they contribute
data.pca$loadings[, 1:2]
```
6. Create additional plots to gain further insight
```{r scree plot}
fviz_eig(data.pca, addlabels = TRUE)
## Here it is just visualized how much each component explains
```
```{r}
# Graph of the variables
fviz_pca_var(data.pca, col.var = "black")

## altså ved første øjekast, ser den lidt fucked ud men det tror jeg ikke den er 
```
```{r}
## So my guess is that you would just need pc1 and pc2 to do dynamic time warping?
...
dtw_result <- dtw::dtw(p1, p2, keep = TRUE)
...
Bare nogle small notes
## ratio between pc 1 and pc 2 tells you which is more important, when it comes to describing the spread of the data. 

## ICA ##
jl = jointlead
lf = leadfollow
```{r ICA for group 0, jointlead}
A0_jl <- fastICA(matrix_group0_jointlead_A, 2, alg.typ = "parallel", fun = "logcosh", alpha = 1,
             method = "R", row.norm = FALSE, maxit = 200,
             tol = 0.0001, verbose = TRUE)

B0_jl <- fastICA(matrix_group0_jointlead_B, 2, alg.typ = "parallel", fun = "logcosh", alpha = 1,
             method = "R", row.norm = FALSE, maxit = 200,
             tol = 0.0001, verbose = TRUE)
```

```{r ICA for group 0, leadfollow}
A0_lf <- fastICA(matrix_group0_leadfollow_A, 2, alg.typ = "parallel", fun = "logcosh", alpha = 1,
             method = "R", row.norm = FALSE, maxit = 200,
             tol = 0.0001, verbose = TRUE)

B0_lf <- fastICA(matrix_group0_leadfollow_B, 2, alg.typ = "parallel", fun = "logcosh", alpha = 1,
             method = "R", row.norm = FALSE, maxit = 200,
             tol = 0.0001, verbose = TRUE)
```

Hi Kat! these are the variables you will need to do DTW! 
```{r Group 0, extracting individual components}
## jointlead
A0_jl <- A0_jl$S
B0_jl <- B0_jl$S
...
## Dont mind this, i am just plotting the components here from A0 in jointlead just to see how it looks
```{r}
par(mfrow = c(1, 2)) # Set up a 1x2 plotting grid for two components
## plotting the first independent component for A
A0_jl[, 1] %>%
  plot()
## plotting the second independent component for B
A0_jl[, 2] %>%
  plot()
...
## limit reached
dtw_result <- dtw::dtw(A0_jl, B0_jl, keep = TRUE) ## min computer siger limit reached, so we are missing some computational strength. Jeg tror sagtens man kan køre den, hvis man f.eks. tager de 10 første sekunder - men vi skal jo gerne have alle 30 sekunder med 

...
### Plotting the data in order to trim the trajectories (fail)

```{r installing plotly}
## Downloading plotly here because it masks some of the functions used earlier, if i download it in the beginning
...
group8_leadfollow_sub <- group8_leadfollow %>%
   dplyr::filter(elapsed_time <= 3)
```

```{r making subject columns into a factor, so we can color by subject later}
## checking class
...
We tried trimming the trajectories by plotting the data in 3d, but due to constraint in computational power, we decided to not proceed with this option, instead we use the linear gap-filling plots to trim the trajectories.

...
wd <- getwd()
if (basename(wd) != "PerceptionActionExam-Clean-up-attempt") {
  setwd("./PerceptionActionExam-Clean-up-attempt")
}
...
```
## naDia:The time series in one plot
```{r}
...
```
### DYNAMITC TIME WARPING BITCHESSSSSS ######

## Computing DTW for each group with a for loop, resulsts can be found in "dtw_results_df"
```{r}
...
NOW I AM JUST PLOTTING IT

Alright, so after sitting with it for a while, i figured out there are 4 types of plots, we can make for dynamic time warping:
1. twoway
2. threeway
3. alignment
4. density

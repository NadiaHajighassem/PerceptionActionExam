...
Now we visualise the components, to make it clear which representations of motion each subject has.
...
Now we get each components per condition into a common plot, effectively making it easier to visualise their pathways as a last step before DTW

```{r}
# Assuming component_list_A and component_list_B are lists of independent components
component_A <- component_list_A[[1]]
component_B <- component_list_B[[1]]

# Find the minimum number of rows between the two components
min_rows <- min(nrow(component_A), nrow(component_B))

# Combine both components into a single dataframe with the minimum number of rows
...
# Creating a time series plot for both subjects
ggplot(combined_df, aes(x = time, y = value, color = group)) +
  geom_line(size = 1) +
  theme_minimal() +
  labs(title = "Independent Components - Subjects A and B", x = "Time", y = "Component Value")
...
This was just to check manually, that the for loop calculated DTW correctly

```{r}
## Doing DTW
dtw_result <- dtw::dtw(component_list_A[[9]], component_list_B[[9]], keep = TRUE)
...
Taking the mean of DTW for all groups to get a baseline
...
Checking which groups are below the mean and therefore high in similariy
...
Alright, so after sitting with it for a while, I figured out there are 4 types of plots, we can make for dynamic time warping: 1. twoway 2. threeway 3. alignment 4. density

```{r twoway}
plot(dtw(component_list_A[[9]],
         component_list_B[[9]],
         k=TRUE),
         type="twoway",
         off=1,
         match.lty=2,
         match.indices=20) ## this argument makes the plot look very nice, but is not a true representation of the step pattern sadly, try removing it and see what happens
```
...
```{r}
plot(
    dtw(component_list_A[[9]],component_list_B[[9]],
        keep=TRUE,
